...
class RemoteFullRouteInformationRepositoryImpl @Inject constructor(
    ...
    override suspend fun getFullRouteInformation(key : String , lineCode: String): Flow<DomainAllRouteInformation> =
        flowOf(service.getFullRouteInformation(key , "json" , "01" , lineCode))
            .map {
                val timeTableMapper = BaseMapper(RemoteAllRouteInformation::class, DomainAllRouteInformation::class)
                val headerMapper = BaseMapper(Header::class, com.young.domain.model.Header::class)
                val bodyMapper = BaseMapper(AllRouteInformation::class, com.young.domain.model.AllRouteInformation::class)
